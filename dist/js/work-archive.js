...
gsap__WEBPACK_IMPORTED_MODULE_0__.gsap.to("#bar-one", _objectSpread(_objectSpread({}, parallaxBarsConfig), {}, {
  yPercent: -400
}));
gsap__WEBPACK_IMPORTED_MODULE_0__.gsap.to("#bar-two", _objectSpread(_objectSpread({}, parallaxBarsConfig), {}, {
  yPercent: -300
}));
}();
// This entry need to be wrapped in an IIFE because it need to be isolated against other entry modules.
...
function initializeGSAP() {
  gsap__WEBPACK_IMPORTED_MODULE_0__.gsap.registerPlugin(gsap_ScrollTrigger__WEBPACK_IMPORTED_MODULE_1__.ScrollTrigger);
  gsap_ScrollTrigger__WEBPACK_IMPORTED_MODULE_1__.ScrollTrigger.batch("#posts-container .article-card", {
    batchMax: 6,
...
jQuery(document).ready(function ($) {
  ...
  var sectors = [];
  var services = [];
  var currentPostsPerPage = getPostsPerPage();
  var params = new URLSearchParams(window.location.search);
  var initialSectorTerm = params.get('sectors') || '';
  var initialServiceTerm = params.get('services') || '';
  function loadPosts() {
    if (isLoading) return;
    isLoading = true;
    $('#loading-indicator').show();
    var postsPerPage = getPostsPerPage();
    $.ajax({
      url: frontendajax.ajaxurl,
      data: {
        action: 'load_our_work_posts',
        sector: sectors.join(','),
  ...
  function updatePosts() {
    ...
    // Collect selected sectors and services
    $('.selected-terms .selected-term').each(function () {
      var term = $(this);
      var value = term.data('value');
      if (term.hasClass('sector')) {
        sectors.push(value);
      } else if (term.hasClass('service')) {
        services.push(value);
      }
    });
    loadPosts();
  ...
  function preselectDropdownOptions() {
    function selectTerm(term, isSector) {
      var value = term.data('value');
      var text = term.text();
      $('.selected-terms').append('<span class="selected-term' + (isSector ? ' sector' : ' service') + '" data-value="' + value + '">' + text + '</span>');
      term.hide();
    }
    if (initialSectorTerm) {
      var sectorTerm = $(".term-selector-item.sector[data-value='" + initialSectorTerm + "']");
      if (sectorTerm.length) {
        sectors.push(initialSectorTerm);
        selectTerm(sectorTerm, true);
      }
    }
    if (initialServiceTerm) {
      var serviceTerm = $(".term-selector-item.service[data-value='" + initialServiceTerm + "']");
      if (serviceTerm.length) {
        services.push(initialServiceTerm);
        selectTerm(serviceTerm, false);
      }
    }
  }
  $(window).resize(function () {
    var newPostsPerPage = getPostsPerPage();
  ...
  $('.term-selector').click(function (event) {
    var termSelector = $('.term-selector');
    var selectorMenuContainer = $('.term-selector-menu-container');
    var selectorTop = termSelector.offset().top;
    var selectorBottom = selectorTop + selectorMenuContainer.outerHeight() + 60;
    ...
    if (selectorTop < windowScrollTop) {
      ...
      var necessaryScroll = selectorBottom - $(window).height() + 30;
  ...
  $('.term-selector-item').click(function () {
    ...
    var text = item.text();
    var isSector = item.hasClass('sector');
    $('.selected-terms').append('<span class="selected-term' + (isSector ? ' sector' : ' service') + '" data-value="' + value + '">' + text + '</span>');
    item.hide();
  ...
  $('.selected-terms').on('click', '.selected-term', function () {
    ...
    var value = selectedTerm.data('value');
    selectedTerm.remove();
    $('.term-selector-item[data-value="' + value + '"]').show();
    updatePosts();
  });
  $(document).click(function (event) {
    var $trigger = $(".term-selector, .term-selector-menu-container");
